...
void Volume4D::setToStep(int step) {
    ...
    oc->isoStep = 15;
    oc->currentIso = 254550 + step * (oc->isoStep);
    //    oc->isoStep = 0.6;
    //    oc->currentIso = 1.5 + step * (oc->isoStep);
...
//    }
    ...
    for (int k = 0; k < oc->zDim; k++) {
        for (int j = 0; j < oc->yDim; j++) {
            for (int i = 0; i < oc->xDim; i++) {
                if (k < 1 || k > oc->zDim - 2 || j < 1 || j > oc->yDim - 2 || i < 1 || i > oc->xDim - 2) {
                    // if (k < 3 || k > oc->zDim - 4 || j < 3 || j > oc->yDim - 4 || i < 3 || i > oc->xDim - 4) {
//
                    vtkVol->SetScalarComponentFromFloat(i, j, k, 0, floatData[voxCount++] / 2);
                    floatData[voxCount - 1] = floatData[voxCount - 1]/2;   
                } else {
                   vtkVol->SetScalarComponentFromFloat(i, j, k, 0, floatData[voxCount]);
                   //floatData[voxCount] =0;
                    int randInt = (int) (((float) rand()*1000) / RAND_MAX);
                    if (k > 30 && k < 65 && randInt < 30 && j > 3 && j < (oc->xDim - 3) && i > 3 && i < (oc->xDim - 3)) {
                        vtkVol->SetScalarComponentFromFloat(i, j, k, 0, 4);
                        floatData[voxCount] = 4;
                    }

                    if (k > 80 && k < 85 && j > 10 && j < (oc->xDim - 10) && i > 10 && i < (oc->xDim - 10)) {
                        vtkVol->SetScalarComponentFromFloat(i, j, k, 0, oc->currentIso + 50);
                        floatData[voxCount - 1] = oc->currentIso + 50;
                        //cout << "created at " << oc->currentIso + 50 << endl;
                    }
                    voxCount++;
                }
            }
    ...
    oc->isoRangeThreshold = oc->isoRange / 20;
...
void Volume4D::findCritcalPoints() {
    ...
    addPersistentBranches(&data, root, oc->isoRangeThreshold, root);
...
void Volume4D::addPersistentBranches(Data* data, ctBranch* b, float thresh, ctBranch* rootBranch) {
    ...
    if (persistence > thresh) {
        ...
        data->convertIndex(extId, x, y, z);
        float rootPersistence = evalPersistence(data, rootBranch);
        //if (persistence <= rootPersistence) {
        {
            //cout <<"pers="<<persistence<<endl<<"rootp="<<rootPersistence<<endl;
...
//                      cout << "1add new CP " << lastPoint->getXFloat() << " " << lastPoint->getYFloat() << " " << lastPoint->getZFloat() << " " << lastPoint->value << endl;
            }
            data->convertIndex(sadId, x, y, z);
            if (!(x == 0 || x == oc->xDim || y == 0 || y == oc->yDim || z == 0 || z == oc->zDim)) {
                sval = vtkVol->GetScalarComponentAsFloat(x, y, z, 0);
                ...
                CriticalPoint* lastPoint = criticalPoints->back();
//                   cout << "2add new CP " << lastPoint->getXFloat() << " " << lastPoint->getYFloat() << " " << lastPoint->getZFloat() << " " << lastPoint->value << endl;
            }
        }

        for (ctBranch * c = b->children.head; c != NULL; c = c->nextChild) {
            addPersistentBranches(data, c, thresh, rootBranch);
        }
    }
